...
public class FunctionalityDAO extends GenericDAO {
	...
	public List<Functionality> searchFunctionalities(String atributo, String termo) throws DBConnectException, DBConsultException {
		String sql = "SELECT " + 
				"	f.id, " + 
				"	f.nome, " + 
				"	f.descricao, " +
				...
				"FROM FUNCIONALIDADE f INNER JOIN PLUGIN p ON f.PLUGINID = p.ID WHERE ?1 LIKE '%?2%'";
	...
	public void createFunctionality(Functionality func) throws DBConnectException, DBCreateException {
		String sql = "INSERT INTO FUNCIONALIDADE (nome, descricao, pluginId, dataCriacao) VALUES ('?1', '?2', ?3, TO_DATE('?4', 'YYYY-MM-DD'))";
		sql = sql.replace("?1", func.getName());
		sql = sql.replace("?2", func.getDescription());
		sql = sql.replace("?3", func.getPlugin().getId().toString());
		sql = sql.replace("?4", func.getDataCriacaoToString());
	...
	public void updateFunctionality(Functionality func) throws DBConnectException, DBUpdateException {
		String sql = "UPDATE FUNCIONALIDADE SET nome = '?1', descricao = '?2', pluginId = ?3 WHERE ID = ?4";
		sql = sql.replace("?1", func.getName());
		sql = sql.replace("?2", func.getDescription());
		sql = sql.replace("?3", func.getPlugin().getId().toString());
		sql = sql.replace("?4", func.getId().toString());
		try { runCreateUpdateDeleteSQL(sql); } 
	...
	public void deleteFunctionality(Long funcId) throws DBConnectException, DBDeleteException {
		String sql = "DELETE FROM FUNCIONALIDADE WHERE id = ?1";
